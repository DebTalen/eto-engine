#ifndef ETO_COMPONENTPOOL_HPP
#define ETO_COMPONENTPOOL_HPP
...
public:
	 ...
	 */
	ComponentPool(size_t reserve = 64);
...
private:
	ComponentPool(const ComponentPool<T> &rhs) = delete;

	ComponentPool<T>& operator=(const ComponentPool<T> &rhs) = delete;
...
template <typename T>
ComponentPool<T>::ComponentPool(size_t reserve)
...
template <typename T>
template <typename ...Args>
T& ComponentPool<T>::add(size_t id, Args&& ...args)
{
...
template <typename T>
T& ComponentPool<T>::get(size_t id) 
{
...
template <typename T>
void ComponentPool<T>::remove(size_t id) 
{
	ETO_ASSERT(id < m_indexes.size()); // existing id
	ETO_ASSERT(m_indexes[id] != -1);   // memory block is not free
...
template <typename T>
int ComponentPool<T>::next_index() 
{
	if (m_free.size())
	{
